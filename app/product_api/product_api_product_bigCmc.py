...
import datetime,time,re,sys,os,threading,functools,html
...
class Product_API:
    ...
    def get_product_details(self,productid):
        ...
        for item in products:
            ...
            bullet_point = '\'\'\''+self.formate_string(bullet_point)+'\'\'\''
            #attributes = item.get_attribute('')
            #browse_nodes = item.browse_nodes
            totalNew = self.formate_string(item.TotalNew)
            if totalNew == '':
                totalNew = '0'
            #image = self.formate_string(item.large_image_url)
            #images = self.formate_string(item.images)
            #print(iamges)
            ...
            sales_rank = self.formate_string(item.sales_rank)
            prime_price = self.formate_string(item.price_and_currency[0])
            ...
            #    data.append('\t'.join([asin,title,manufacturer,brand,image,sales_rank,availability,product_group,binding,weight,size,totalNew,isEligibleForPrime,browses_name,bullet_point,description,color,prime_price,upc,length,width,height]))
            data.append('\t'.join([asin,title,manufacturer,brand,image,sales_rank,availability,product_group,binding,weight,size,totalNew,isEligibleForPrime,browses_name,bullet_point,description,color,prime_price,upc,length,width,height,MetalType,MaterialType,review]))
            #     break
            # except:
            #     #data=[productid]
